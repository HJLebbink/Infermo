...
struct Conv2d:
    ...
    fn __init__(
        ...
        self.kernels = nn.tensor(
            shape(out_channels, in_channels, kernel_width, kernel_height)
        )
        self.kernels.randHe()
...
struct Mlp:
    ...
    fn __init__(
        inout self,
        inout nn: Module,
        neurons_per_layer: DynamicVector[Int],
        add_bias: Bool = True,
        activation: String = "relu",
    ):
...
fn accuracy(logits: Tensor, true_vals: Tensor) -> Float32:
    ...
    for i in range(true_vals.cap):
        if logits.data.load(i) == Float32(1.0) and true_vals.data.load(i) == Float32(
            1.0
        ):
            avg_acc += 1
